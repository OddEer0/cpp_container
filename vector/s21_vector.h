...
namespace s21 {
    ...
    public:
        ...
        private:
            ...
            bool isReverse_;
        public:
            Iterator() : vector_(nullptr), index_(0), position_(START), isReverse_(false) {}
            Iterator(vector *v, size_type index, int pos, bool isReverse = false) : vector_(v), index_(index), position_(pos), isReverse_(isReverse) {};
            ~Iterator() {
                vector_ = nullptr;
                index_ = 0;
                position_ = START;
        ...
        public:
            vector() : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {};
            vector(size_type count, const T& value)  : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {
                resize(count);
                for (size_type i = 0; i < count; ++i) {
                    push_back(value);
                }
            };
            vector(size_type count) : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {
                resize(count);
            }
            vector(const vector& other) : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {
                clear();
            ...
            vector(vector&& other) : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {
                clear();
                if (other.data_ != nullptr) {
                    resize(other.capacity_);
                    for (size_type i = 0; i < other.length_; ++i) {
            ...
            vector(std::initializer_list<T> init) : data_(nullptr), length_(0), capacity_(0), allocator_(Allocator()) {
                for (auto item : init) {
                    push_back(item);
                }
            }
            ...
            std::string stringWithCallback(std::function<std::string(value_type)> callback) {
                if (length_ == 0) {
                    return "empty";
                }
                std::string str = "";
            ...
            // Iterator
            iterator begin() {
                return Iterator(this, 0, PROCESS);
            }
            ...
            iterator rend() {
                return Iterator(this, 0, START, true);
            }
