...
template <class Key, class Compare, class Allocator>
std::string RedBlackTree<Key, Compare, Allocator>::stringWithCompare(std::function<std::string(key_type)> callback, bool withColor) {
    std::string str = "";
    if (!empty()) {
...
template <class Key, class Compare, class Allocator>
void RedBlackTree<Key, Compare, Allocator>::outputWithCompare(node_type node, std::string prefix, bool isTail, std::string* str, std::function<std::string(key_type)> callback, bool withColor) {
    if (node->right_ != nullptr) {
        std::string newPrefix = prefix;
        if (isTail) {
            newPrefix += "│   ";
